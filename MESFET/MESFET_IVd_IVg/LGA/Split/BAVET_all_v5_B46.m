...
% %+++++++++++++++++ Fit I-VD for Vg=0 to -1V +++++++++++++++++++++++++++  
  %get Ron, turn-on voltage, output conductance
    for i_column2=1:1:2  % for Vg of 0V and -1 V  
        ...
        i_X_end1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.2))); % 0.85index when Vd > 2V
        ...
        i_X_start2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.48))); % 2index when Vd > 1.5V
        i_X_end2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 1.4))); % 3.4index when Vd > 2V
...
%+++++++++++++++++ Fit I-VD for Vg=-2V +++++++++++++++++++++++++++
      % Vdsat, Ron and Gout Fit for Vgs = -2 V
        ...
        i_X_end1{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 0.2))); % 2.2index when Vd > 2V
        ...
        i_X_start2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 0.15))); % 6 or 7.5 index when Vd > 1.5V
        i_X_end2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 0.8))); % 9.5 or 10index when Vd > 2V
...
%+++++++++++++++++ Fit I-Vg for Vd of 4,5,6V +++++++++++++++++++++++++++
                    ...
                    i_X_start4{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -2.8))); % % -3.3index when Vd > 1.5V
                    i_X_end4{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -4.2))); % -4.5index when Vd > 2V
                    ...
                    i_X_end5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -1.2))); % -1.8index when Vd > 2V
...
%+++++++++++++++++ Fit low I-Vg for Vd of 4V +++++++++++++++++++++++++++                    
                    ...
                    z7=1
                     % Fit for low Vgs to get Vth1
                    i_X_start6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -2.8)));  % -3.3index when Vd > 1.5V
                    i_X_end6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -4.2))); % -4.5index when Vd > 2V
