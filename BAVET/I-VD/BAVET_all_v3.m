...
% 
% 
clear X Y1 Y2 Y3 i_column i_plot p1 p2 p3 p4 p5 i_data count  control_Var_old  control_Var_new dI dI_max dI_min i_Vdstart i_Vdend
clear VdminperVg iVdminperVg Legend_name PlotLegend1 V_turnon1 V_turnon2 R_on Xmax_percontrol_var iXmax_percontrol_var Y1max_percontrol_var iY1max_percontrol_var
clear Y2min_percontrol_var iY2min_percontrol_var Y3min_percontrol_var iY3min_percontrol_var xlabel xlabelC ylabel ylabelC TitelStr1
...
%   
  %get Ron, turn-on voltage, output conductance
    for i_column2=1:1:3  % for Vg of 0V and -1 V  
        ...
        i_X_start1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.25))); % index when Vd > 1.5V
        i_X_end1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.85))); % index when Vd > 2V
        ...
        i_X_start2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 2))); % index when Vd > 1.5V
        i_X_end2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 3))); % index when Vd > 2V
        ...
        i_X_start3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 6))); % index when Vd > 1.5V
        i_X_end3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 6.99))); % index when Vd > 2V
...
%                     set(Ax1,'fontsize',25,'FontName','Times New Roman')
%             %        Legend_eachC= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
%                     TitleStr1=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/',sample_info{1,9},') ',sample_info{1,5},' ', sample_info{1,8},' ',': L_{AP} = ', num2str(Lap),' ',char(0181),'m, L_{GO}= ',num2str(Lgo),' ',char(0181),'m, Lga= ',num2str(L_{GA}),' ',char(0181),'m, W = (2 x', num2str(Wap),char(0181),'m) ', control_label,' = ', num2str(Control_Var_Start), ' V to ', num2str(Control_Var_End), ' V in steps of ', num2str(control_Var_step), ' V']
%                     %set(TitleStr1,'interpreter','tex') 
%                     title(TitleStr1, 'Units', 'normalized', ...
%                          'Position', [0.5 1.02],'FontSize',20,'FontName','Times New Roman', 'interpreter','tex')       
%             %       
%                   %Ends - figure settings
%                   hold on; 
%                   p1=plot(X(1:iXmin_percontrol_var(i_column),i_column),Y1_density(1:iXmin_percontrol_var(i_column),i_column),PlotLineSpec{1},'LineWidth',2);  
%                   p2=plot(X(1:iXmin_percontrol_var(i_column),i_column),abs(Y3_density(1:iXmin_percontrol_var(i_column),i_column)),PlotLineSpec{2},'LineWidth',1);  
%                   p3=plot(X(1:iXmin_percontrol_var(i_column),i_column),abs(Y2_density(1:iXmin_percontrol_var(i_column),i_column)),PlotLineSpec{3},'LineWidth',1);  
%                   hold on;
%             %       %check if Id+Is+Ig =0%
%             %       dI(1:iVdmaxperVg(i_column),i_column) = Y1(1:iVdmaxperVg(i_column),i_column)+ Y3(1:iVdmaxperVg(i_column),i_column)+ Ig(1:iVdmaxperVg(i_column),i_column)
%             %       dI_max = max(dI)
