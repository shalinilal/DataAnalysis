     ...
     %             p3b=plot(X(1:iXmin_percontrol_var(i_column),i_column),abs(Y2_density(1:iXmin_percontrol_var(i_column),i_column)),PlotLineSpec{3},'LineWidth',1);  
            ...
            %       dI_min = min(dI)
                  %%
                    ...
                    for i_column3=1:1:4  % for Vg of 0V and -1 V  
                    z6=1
                     % Fit for low Vgs to get Vth1
                    i_X_start4{i_column3} = min(find((X(1:iXmax_percontrol_var(i_column3),i_column3) < -3.3))); % index when Vd > 1.5V
                    i_X_end4{i_column3} = min(find((X(1:iXmax_percontrol_var(i_column3),i_column3) < -4.5))); % index when Vd > 2V
                    ...
                    % Ends - Fit for mid Vds to get Vturn-on2
                    i_X_start5{i_column3} = min(find((X(1:iXmax_percontrol_var(i_column3),i_column3) < -0.8))); % index when Vd > 1.5V
                    i_X_end5{i_column3} = min(find((X(1:iXmax_percontrol_var(i_column3),i_column3) < -1.8))); % index when Vd > 2V
