  ...
  %get Ron, turn-on voltage, output conductance
    for i_column2=1:1:2  % for Vg of 0V and -1 V  
      ...
      % Vdsat, Ron and Gout Fit for Vgs = -2 V
        ...
        i_X_start2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 2))); % 6 or 7.5 index when Vd > 1.5V
        i_X_end2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 3.3))); % 9.5 or 10index when Vd > 2V
        ...
        i_X_start3{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 5.5))); % 10.5index when Vd > 1.5V
        i_X_end3{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 6.8))); % 11.9index when Vd > 2V
     ...
     %             p3b=plot(X(1:iXmin_percontrol_var(i_column),i_column),abs(Y2_density(1:iXmin_percontrol_var(i_column),i_column)),PlotLineSpec{3},'LineWidth',1);  
            ...
            %       dI_min = min(dI)
                  %%
                    ...
                    z7=1
                     % Fit for low Vgs to get Vth1
                    i_X_start6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -3.6)));  % -3.3index when Vd > 1.5V
                    i_X_end6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -4.75))); % -4.5index when Vd > 2V
