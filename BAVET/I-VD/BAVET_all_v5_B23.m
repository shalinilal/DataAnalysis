...
%   
  %get Ron, turn-on voltage, output conductance
    for i_column2=1:1:2  % for Vg of 0V and -1 V  
        ...
        i_X_start1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.7))); % 0.4index when Vd > 1.5V
        i_X_end1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 2.2))); % 2.2index when Vd > 2V
        ...
        i_X_start2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 5))); % 6 or 8index when Vd > 1.5V
        i_X_end2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 8))); % 9.5 or 10index when Vd > 2V
        ...
        i_X_start3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 10))); % 10.5index when Vd > 1.5V
        i_X_end3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 11.7))); % 11.9index when Vd > 2V
      ...
      % Vdsat, Ron and Gout Fit for Vgs = -2 V
        % Fit for low Vds to get Vturn-on
        i_X_start1{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 0.7))); % 0.4index when Vd > 1.5V
        ...
        i_X_start2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 3))); % 6 or 7.5 index when Vd > 1.5V
        i_X_end2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 5))); % 9.5 or 10index when Vd > 2V
        ...
        i_X_start3{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 8))); % 10.5index when Vd > 1.5V
    ...
    % Fit for I-Vd when Vg=-6, -5 V 
     for i_column5=6:1:7  % for Vg of -5V and -6 V  
        ...
        % Fit for saturated I-V to get Vdsat and output-conductance
        i_X_start7{i_column5} = min(find((X(1:iXmax_percontrol_var(i_column5),i_column5) > 1.8))); % 4index when Vd > 1.5V
        i_X_end7{i_column5} = min(find((X(1:iXmax_percontrol_var(i_column5),i_column5) > 3))); % 6.99index when Vd > 2V
     ...
     %             p3b=plot(X(1:iXmin_percontrol_var(i_column),i_column),abs(Y2_density(1:iXmin_percontrol_var(i_column),i_column)),PlotLineSpec{3},'LineWidth',1);  
            ...
            %       dI_min = min(dI)
                  %%
                    ...
                    % Ends - Fit for mid Vds to get Vturn-on2
                    i_X_start5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -0.5))); % -1index when Vd > 1.5V
                    i_X_end5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -1.75))); % -2.2index when Vd > 2V
                    ...
                    i_X_end6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -5.5))); % -5index when Vd > 2V
