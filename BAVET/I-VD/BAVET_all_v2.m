...
% 
% 
clear X Y1 Y2 Y3 i_column i_plot p1 i_data count  control_Var_old  control_Var_new dI dI_max dI_min i_Vdstart i_Vdend
        ...
        if X_Var_step > 0 % for X limits
           ...
           Y3_min = min(Y3min_percontrol_var) % find highest Is
            ...
            %        Legend_eachC= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
                    ...
                    TitleStr1=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/',sample_info{1,9},') ',sample_info{1,5},' ', sample_info{1,8},' ',': Lap = ', num2str(Lap),' ',char(0181),'m, Lgo= ',num2str(Lgo),' ',char(0181),'m, Lga= ',num2str(Lga),' ',char(0181),'m, W = (2 x', num2str(Wap),char(0181),'m) ', control_label,' = ', num2str(Control_Var_Start), ' V to ', num2str(Control_Var_End), ' V in steps of ', num2str(control_Var_step), ' V']
                  ...
                  hold on; 
                  p1=plot(X(1:iXmax_percontrol_var(i_column),i_column),Y1_density(1:iXmax_percontrol_var(i_column),i_column),PlotLineSpec{1},'LineWidth',2);  
                  p2=plot(X(1:iXmax_percontrol_var(i_column),i_column),abs(Y3_density(1:iXmax_percontrol_var(i_column),i_column)),PlotLineSpec{2},'LineWidth',1);  
