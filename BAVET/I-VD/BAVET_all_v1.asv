...
clear  ind_cvar ind_xvar Control_Var X_Var wkdir w filenam1 
...
%       end
 ...
 for i=1:length(wk_dir) 
    ...
    Control_Var = FileData{3,1}(1:ind_cvar-1) % control variable name
    X_Var = FileData{2,1}(1:ind_xvar-1)     % x axis variable name
    size(FileData,2)
    header= FileData(4,:)
    
     for i_header=1:1:5 % find columns of control variable like Vg and x_variable like Vd for I-Vd
        if(strcmpi(Control_Var, FileData(4,i_header))== 1)
            control_columnindex = i_header
        elseif  (strcmpi(X_Var, FileData(4,i_header))== 1)  
