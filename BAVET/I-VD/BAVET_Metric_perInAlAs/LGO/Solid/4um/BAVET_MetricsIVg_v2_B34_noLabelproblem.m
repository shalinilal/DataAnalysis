...
% % read device details: name, Lap,Lg,Lgmesa %
 ...
 i_LapSeries=1
  ...
  for i_DevData=1:1:length(DevLookup_name)
      z1=1
     if DevLookup_Lga(i_DevData) == (DevLookup_Lap(i_DevData)/2) && DevLookup_Lgo(i_DevData) == 4 && DevLookup_Lap(i_DevData) >0 % Devices which are solid and with Lgo = 4um, and an aperture
         z2=1
...
%         %TitleStr1=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/) ',sample_info{1,5},' ', sample_info{1,6}]
         TitleStr1=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/',sample_info{1,6},') ',sample_info{1,5}, ' BAVETs with L_{GO}= ', num2str(Lgo_title),' ',char(0181),'m', ' and L_{GA}= (0.5 x L_{AP}) ',char(0181),'m',' ',Y1_label{1,i_Y1inLapSeries+2},' vs. ',X_label, ' for different V_{DS}']
%           
