...
% % read device details: name, Lap,Lg,Lgmesa %
 ...
 i_LapSeries=1
  ...
  for i_DevData=1:1:length(DevLookup_name)
      z1=1
     if DevLookup_Lga(i_DevData) == (DevLookup_Lap(i_DevData)/2) && DevLookup_Lgo(i_DevData) == 2 && DevLookup_Lap(i_DevData) >0 % Devices which are solid and with Lgo = 2um, and an aperture
         z2=1
