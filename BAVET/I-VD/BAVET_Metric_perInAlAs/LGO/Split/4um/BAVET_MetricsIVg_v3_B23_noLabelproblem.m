...
% % read device details: name, Lap,Lg,Lgmesa %
 ...
 i_LapSeries=1
  ...
  for i_DevData=1:1:length(DevLookup_name)
      z1=1
     if DevLookup_Lga(i_DevData) == 0 && DevLookup_Lgo(i_DevData) == 4 && DevLookup_Lap(i_DevData) >0 % Devices which are split and with Lgo = 4um, and an aperture
         z2=1
...
for i_LapSeries1 =1:1:(size(NaN_values_TF,1)) 
...
for i_LapSeries2 =1:1:size(LapSeries,1)  
...
%no_of_Vc{1,i_col_metric-2}=0
    for i_Vc_row =1:1:Vc_row
        for i_LapSeries3 =1:1:size(NaN_values_TF,1)  
...
for i_Lap_noofVc=1:1:size(no_of_Vc,1)
