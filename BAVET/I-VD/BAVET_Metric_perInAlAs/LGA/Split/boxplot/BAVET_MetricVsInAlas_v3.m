...
%     
     ...
     Y_boxaxislabel = strcat(Metric_Data{1,2}(1:1),'_{',Metric_Data{1,2}(2:end),'} (', Y1_units{1,3},')') % Yaxis label is the metric name
...
%         LegendStrY1{i_Xcol}=sprintf('%s',Legend_eachY1 ) 
  ...
  %  m='pInAlas'
    ...
    set(Ax1,'xtick',1:1:3)
        set(Ax1,'xticklabel',Y1_label(3:5),'FontSize',20,'FontName','Times New Roman')
        ylabel(get(Ax1,'Ylabel'),'String',Y_boxaxislabel,'FontSize',25,'FontName','Times New Roman')
    %Median
    h_median = findobj(gcf,'tag','Median')
    x_median= get(h_median,'XData');
    y_median= get(h_median,'YData');
    
    %25th Percentile and 75th
    h_25thper = findobj(gcf,'tag','Box')
    x_25thper= get(h_25thper,'XData');
    y_25thper= get(h_25thper,'YData');
    
%     %75th Percentile
%     h_75thper= findobj(gcf,'tag','Upper Whisker')
%     x_75thper= get(h_75thper,'XData');
%     y_75thper= get(h_75thper,'YData');
%     
