...
%       end
  ...
  clear Ax1 Ax2 X Y1 Y2 Y3 i_column i_plot p1 p2 p3 p1b p2b p3b p4 p5 p6 p7 i_data count  control_Var_old  control_Var_new dI dI_max dI_min i_Vdstart i_Vdend
...
%   
  %get Ron, turn-on voltage, output conductance
    for i_column2=1:1:3  % for Vg of 0V and -1 V  
        z5=1
        
        % Fit for low Vds to get Vturn-on
        i_X_start1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.42))); % 0.25index when Vd > 1.5V
        i_X_end1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.9))); % 0.85index when Vd > 2V
        ...
        i_X_start2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 2))); % 2index when Vd > 1.5V
        i_X_end2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 3.4))); % 3.4index when Vd > 2V
        ...
        i_X_start3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 5.5))); % 6index when Vd > 1.5V
        i_X_end3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 6.99))); % 6.99index when Vd > 2V
     ...
     %             p3b=plot(X(1:iXmin_percontrol_var(i_column),i_column),abs(Y2_density(1:iXmin_percontrol_var(i_column),i_column)),PlotLineSpec{3},'LineWidth',1);  
            ...
            %       dI_min = min(dI)
                  %%
                    ...
                    for i_column3=1:1:3  % for Vg of 0V and -1 V  
                    z6=1
                     % Fit for low Vgs to get Vth1
                    i_X_start4{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -3.6))); % -3.3index when Vd > 1.5V
                    i_X_end4{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -4.75))); % -4.5index when Vd > 2V
                    ...
                    % Ends - Fit for mid Vds to get Vturn-on2
                    i_X_start5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -0.7))); % -0.8index when Vd > 1.5V
                    i_X_end5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -1.8))); % -1.8index when Vd > 2V
