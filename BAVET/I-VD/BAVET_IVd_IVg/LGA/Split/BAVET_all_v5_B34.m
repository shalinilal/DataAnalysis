        ...
        if X_Var_step > 0 % for X limits
           ...
           Y3_min = min(Y3min_percontrol_var) % find highest Is
            ...
            %        Legend_eachC= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
                    ...
                    TitleStr1=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/',sample_info{1,9},') ',sample_info{1,5},' ', sample_info{1,8},' ',': L_{AP} = ', num2str(Lap),' ',char(0181),'m, L_{GO}= ',num2str(Lgo),' ',char(0181),'m, L_{GA}= ',num2str(Lga),' ',char(0181),'m, W = (2 x ', num2str(Wap),char(0181),'m) ', control_label,' = ', num2str(Control_Var_Start), ' V to ', num2str(Control_Var_End), ' V in steps of ', num2str(control_Var_step), ' V']
...
% %+++++++++++++++++ Fit I-VD for Vg=0 to -1V +++++++++++++++++++++++++++     
  %get Ron, turn-on voltage, output conductance
    for i_column2=1:1:2  % for Vg of 0V and -1 V  
        ...
        i_X_start1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 0.6))); % 0.4index when Vd > 1.5V
        i_X_end1{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 2.2))); % 1.6index when Vd > 2V
        ...
        i_X_start2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 7))); % 4.5 or 8index when Vd > 1.5V
        i_X_end2{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 10))); % 7 or 10index when Vd > 2V
        ...
        i_X_start3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 11))); % 10.5index when Vd > 1.5V
        i_X_end3{i_column2} = min(find((X(1:iXmax_percontrol_var(i_column2),i_column2) > 11.9))); % 11.9index when Vd > 2V
...
%+++++++++++++++++ Fit I-VD for Vg=-2V +++++++++++++++++++++++++++
      % Vdsat, Ron and Gout Fit for Vgs = -2 V
        % Fit for low Vds to get Vturn-on
        i_X_start1{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 0.6))); % 0.4index when Vd > 1.5V
        i_X_end1{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 2.2))); % 2.2index when Vd > 2V
        ...
        i_X_start2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 5))); % 4 or 7.5 index when Vd > 1.5V
        i_X_end2{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 8))); % 6.8 or 10index when Vd > 2V
        ...
        i_X_start3{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 10))); % 9index when Vd > 1.5V
        i_X_end3{3} = min(find((X(1:iXmax_percontrol_var(3),3) > 11.9))); % 11.9index when Vd > 2V
...
%+++++++++++++++++ Fit I-VD for Vg=-5,-6V +++++++++++++++++++++++++++    
    % Fit for I-Vd when Vg=-6, -5 V 
     for i_column5=6:1:7  % for Vg of -5V and -6 V  
        z5=1
              
        % Fit for saturated I-V to get Vdsat and output-conductance
        i_X_start7{i_column5} = min(find((X(1:iXmax_percontrol_var(i_column5),i_column5) > 8))); % 5index when Vd > 1.5V
        i_X_end7{i_column5} = min(find((X(1:iXmax_percontrol_var(i_column5),i_column5) > 9))); % 8index when Vd > 2V
        fitY3X7{i_column5} = polyfit(X(i_X_start7{i_column5}:i_X_end7{i_column5},i_column5),abs(Y3_density(i_X_start7{i_column5}:i_X_end7{i_column5},i_column5)),1);
        x_fit7= 4:0.2:12;
        y_fit7 = fitY3X7{i_column5}(2)+x_fit7*fitY3X7{i_column5}(1); %making a line to get 
    ...
    %Ends Fit for I-Vd when Vg=-6, -5 V 
    ...
    set(PlotLegend1,'YColor',[1 1 1],'XColor',[1 1 1],...
        'FontSize',22,'FontName','Times New Roman');
            ...
            %        Legend_eachC= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
                    TitleStr2=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/',sample_info{1,9},') ',sample_info{1,5},' ', sample_info{1,8},' ',': L_{AP} = ', num2str(Lap),' ',char(0181),'m, L_{GO}= ',num2str(Lgo),' ',char(0181),'m, L_{GA}= ',num2str(Lga),' ',char(0181),'m, W = (2 x ', num2str(Wap),char(0181),'m) ', control_label,' = ', num2str(Control_Var_Start), ' V to ', num2str(Control_Var_End), ' V in steps of ', num2str(control_Var_step), ' V']
                    %set(TitleStr1,'interpreter','tex') 
...
%+++++++++++++++++ Fit I-Vg for Vd of 9,10,11V +++++++++++++++++++++++++++                  
                    for i_column3=5:1:7  % for Vd of 9,10,11V  
                    z6=1
                     % Fit for low Vgs to get Vth1
                    i_X_start4{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -4.9))); % -5index when Vd > 1.5V
                    i_X_end4{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -5.9))); % -5.9index when Vd > 2V
                    ...
                    % Ends - Fit for mid Vds to get Vturn-on2
                    i_X_start5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -1.4))); % -1.6index when Vd > 1.5V
                    i_X_end5{i_column3} = min(find((X(1:iXmin_percontrol_var(i_column3),i_column3) < -2.8))); % -2.8index when Vd > 2V
...
%+++++++++++++++++ Fit low I-Vg for Vd of 5,6,7V +++++++++++++++++++++++++++                     
                    % Fit for I-Vg when low Vds (5-7V)
                    for i_column4=1:1:3  % for Vg of 0V and -1 V  
                    z7=1
                     % Fit for low Vgs to get Vth1
                    i_X_start6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -4.9))); % -5index when Vd > 1.5V
                    i_X_end6{i_column4} = min(find((X(1:iXmin_percontrol_var(i_column4),i_column4) < -5.8))); % -5.9index when Vd > 2V
