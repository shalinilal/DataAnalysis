...
%no_of_Vc{1,i_col_metric-2}=0
    for i_Vc_row =1:1:Vc_row
        for i_LapSeries3 =1:1:length(NaN_values_TF)  
            if NaN_values_TF(i_LapSeries3,i_col_metric2) ==0 &&  no_of_Vc(Vc_row,1)== LapSeries(i_LapSeries3,1)
...
for i_LapSeries=1:1:length(LapSeries)
   ...
   % for i_Y1_col =3:1:3% size(no_of_Vc,2) %creates columns for every Y variable like Vds or Id or Ron etc, and puts data for each Vg in each column. 
            % No. of columns created for each Y will depend on the no_of_Vc
            % for that Y and Lap
    % if NaN_values_TF(i_LapSeries,1+i_Y1_col) ==0 %&&  no_of_Vc(i_row,1)== LapSeries(i_LapSeries,1)
%        for i_no_of_Vc_perY1col=1:1:no_of_Vc(i_row,i_Y1_col)
%        i_col_Y1_perVc = ((i_Y1_col-1)+i_no_of_Vc_perY1col)
%        Y1(i_row,i_col_Y1_perVc)=  LapSeries(i_LapSeries,1+i_Y1_col); 
       
     %else   i_col_Y1_perVc = count+ (i_Y1_col*no_of_Vc(i_row,i_Y1_col))
