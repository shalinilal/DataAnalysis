...
%      end
    ...
    if FileData{2,1}(1:end-1) == 'VG'
        Vg_step = str2double(FileData{2,6})
         if str2double(FileData{2,2}) > str2double(FileData{2,4}) % find Vdmax and Vdmin, depends on how the measurement was swept
               Vg_min= str2double(FileData{2,4})
               Vg_max= str2double(FileData{2,2})         
        else
               Vg_min= str2double(FileData{2,2})
               Vg_max= str2double(FileData{2,4})
        end 
    end
       i_PlotData =1;
    ...
    if FileData{3,1}(1:end-1) == 'VDS'
        Vd_step = str2double(FileData{3,6})
        if str2double(FileData{3,2}) > str2double(FileData{3,4})  % find Vdmax and Vdmin, depends on how the measurement was swept
               Vd_min= str2double(FileData{3,4})
               Vd_max= str2double(FileData{3,2})         
        else
               Vd_min= str2double(FileData{3,2})
               Vd_max= str2double(FileData{3,4})
        end    
    end
    


clear Vd Id Ig Is iVg Vg i_column i_plot p1 i_data count  Vg_old  Vg_new dI dI_max dI_min i_Vdstart i_Vdend
clear VgminperVd iVgminperVd Legend_name PlotLegend1 V_turnon R_on fitJdVg x_fit y_fit i_Vgend i_Vgstart
...
i_row =0
 ...
 count = 1;
    ...
    for i_data=1:1:length(PlotData)
            ...
            Vg(i_row,count)=  PlotData(i_data,3);
            [VgminperVd(count), iVgminperVd(count)]= min(Vg(:,count))
  ...
  %
TitleStr1=[sample_info{1,2},' (',sample_info{1,4},'-InAlAs/InGaAs/',sample_info{1,9},') ',sample_info{1,5},' ', sample_info{1,8},' ',': Lap = ', num2str(Lap),' ',char(0181),'m, Lgo= ',num2str(Lgo),' ',char(0181),'m, Lga= ',num2str(Lga),' ',char(0181),'m, W = (2 x', num2str(Wap),char(0181),'m) Vds =', num2str(Vd_max), ' V to ', num2str(Vd_min), ' V in steps of ', num2str(Vd_step), ' V']
  for i_column=1:1:size(Vg,2)
      x=1;
      %figure settings
        fig1= figure(1);
...
%       
      ...
      hold on; 
    %  p1=plot(Vg(1:iVgminperVd(i_column),i_column),Jd(1:iVgminperVd(i_column),i_column),PlotLineSpec{1},'LineWidth',2);  
      p2=plot(Vg(1:iVgminperVd(i_column),i_column),abs(Js(1:iVgminperVd(i_column),i_column)),PlotLineSpec{2},'LineWidth',1);  
    %   p3=plot(Vg(1:iVgminperVd(i_column),i_column),abs(Jg(1:iVgminperVd(i_column),i_column)),PlotLineSpec{3},'LineWidth',1);  
      hold on;
      %check if Id+Is+Ig =0%
      dI(1:iVgminperVd(i_column),i_column) =   Id(1:iVgminperVd(i_column),i_column)+ Is(1:iVgminperVd(i_column),i_column)+Ig(1:iVgminperVd(i_column),i_column)
  ...
  %get Ron, turn-on voltage, output conductance
    for i_column=7:1:size(Vg,2)  % for Vd of 4V and 5 V  
        i_Vgend{i_column} = min(find((Vg(1:iVgminperVd(i_column),i_column) < -3.9))); % index when Vg < -1.8V
        i_Vgstart{i_column} = max(find((Vg(1:iVgminperVd(i_column),i_column) > -3.4))); % index when Vg > -0.8V
        fitJdVg{i_column} = polyfit(Vg(i_Vgstart{i_column}:i_Vgend{i_column},i_column),abs(Js(i_Vgstart{i_column}:i_Vgend{i_column},i_column)),1);
        x_fit= -4.2:0.1:-2.0%-3:0.2:0.5;
        y_fit = fitJdVg{i_column}(2)+x_fit*fitJdVg{i_column}(1); %making a line to get 
         V_turnon(i_column)= -1.*(fitJdVg{i_column}(2)/fitJdVg{i_column}(1));
         R_on(i_column) = (1./fitJdVg{i_column}(1)).*(10^3) %in Ohm-mm
        hold on;
%         p3=plot(Vg(1:iVgminperVd(i_column),i_column),Jd(1:iVgminperVd(i_column),i_column),PlotLineSpec{1},'LineWidth',2);  
         p4=plot(x_fit,y_fit,'Color','r','LineWidth',1);
    end
