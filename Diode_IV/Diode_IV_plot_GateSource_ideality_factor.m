...
while m <= (2*no_of_files)   
        ...
        I=(cell2mat(PlotData(:,m+1))/(Gt_Area));
         %Plot I-V
        fig1= figure(1);
        set(gcf,'color','w');
        box on;
...
%         ylim('auto')
        ...
        LegendStrlogI{x}=sprintf('%s',Legend_eachlogI ) 
         index_1V = (find((V == 0.5))); 
         index_2p5V = (find((V == 0.2))); 
         lnI=log(abs(I))
         p = polyfit(V(index_1V:index_2p5V),lnI(index_1V:index_2p5V),1);
         n= 1/(p(1)*0.026);
        x_fit=0:0.2:1.6;
         y_fit = (exp(p(2)).*exp(x_fit*p(1))); %making a line to get 
        PlotLineslogI(x)=semilogy(V,abs(I),PlotLineSpec{x},'LineWidth',2);
        hold on;
        PlotLineslnI(x)=semilogy(x_fit,y_fit,'r');
        title(TitleStrI, 'Units', 'normalized', ...
            'Position', [0.5 1.02],'FontSize',25,'FontName','Times New Roman')
        set(Ax2,'LineWidth',2)
