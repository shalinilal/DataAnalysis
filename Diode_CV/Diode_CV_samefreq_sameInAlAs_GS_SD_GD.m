...
% no = str2num(no_of_files)
 for i_dir = 1:no_of_files
     ...
     filename1{i_dir,:} = input('Input file name: ','s')
 end  
...
%       files = dir(fullfile('wk_dir{1}', 'wk_dir{2}', 'wk_dir{3}', 'R34_SD3.txt'))    
 ...
 wk_dir
   ...
   for i_filename1 = 1:length(filename1)
        for i_devlookup = 1:length(DevLookup)
        if strcmpi(DevLookup(i_devlookup,1), filename1(i_filename1,1:end-4))
            Ap_Width(i_filename1) = str2num(DevLookup{i_devlookup,2}) %in um
            Ap_Area(i_filename1) = (str2num(DevLookup{i_devlookup,2}).* 10^(-4)).^2 % cm2
            Gt_Width(i_filename1) = str2num(DevLookup{i_devlookup,3}) %in um
            Gt_Area(i_filename1) = (str2num(DevLookup{i_devlookup,3}).* 10^(-4)).^2 % cm2
            Ms_Width(i_filename1) = str2num(DevLookup{i_devlookup,4}) % in um
            Ms_Area(i_filename1) = (str2num(DevLookup{i_devlookup,4}).* 10^(-4)).^2 % cm2
        end
    end
   end  
