...
TitleStr1=[sample_info{1,7},' (',sample_info{1,9},') of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
TitleStr2=['G-V (',sample_info{1,9},') of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
TitleStrCinv2=['1/C^{2}-V (',sample_info{1,9},') of ', sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
TitleStrNV=['n-V (',sample_info{1,9},') of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
TitleStrNX=['n-x (',sample_info{1,9},') of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
% 
m=1;
...
while m <= (3*no_of_files) && x <= (no_of_files) 
        ...
        dV = interp1(V,linspace(1,length(V),50));
        dC = interp1(C,linspace(1,length(C),50));
        dG = interp1(G,linspace(1,length(G),50));
...
%         [C2,V2] = smoother(Cm,V,dCmin);
     ...
     %Plot CV%       
        ...
        Legend_eachC=[(sample_info{x,5}),'-InAlAs']  
        LegendStrC{x}=sprintf('%s',Legend_eachC )
        hold on;
        PlotLinesC(x)=plot(dV,dC,PlotLineSpec{x},'LineWidth',2); 
...
%        xlim('auto')
        set(Ax1,'XLim',[-1 7])
        set(Ax1, 'XTick', [-1 0 1 2 3 4 5 6 7])
        set(Ax1,'YColor', 'black')
        ylabelC = ['C (',char(0181),'F/cm^{2})'];
        ylabel(get(Ax1,'Ylabel'),'String',ylabelC,'FontSize',30,'FontName','Times New Roman') 
        xlabel('V_{DS} (V)','FontSize',30,'FontName','Times New Roman') 
        set(Ax1,'fontsize',30,'FontName','Times New Roman')
      %ends - Plot CV%   
...
%        Legend_eachG= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
        Legend_eachG = [(sample_info{x,5}),'-InAlAs']
        LegendStrG{x}=sprintf('%s',Legend_eachG ) 
        PlotLinesG(x)=semilogy(dV,abs(dG),PlotLineSpec{x},'LineWidth',2); 
...
%        ylim('auto')
        ylim([10.^0 10.^3])
        set(Ax2, 'YTick', [10.^0 10.^1 10.^2 10.^3])
%        xlim('auto')
        set(Ax2,'XLim',[-1 7])
        set(Ax2, 'XTick', [-1 0 1 2 3 4 5 6 7])
        set(Ax2,'YColor', 'black')
        set(Ax2,'fontsize',30,'FontName','Times New Roman')
        ylabel('G (Scm^{-2})','FontSize',30,'FontName','Times New Roman') 
        xlabel('V_{DS} (V)','FontSize',30,'FontName','Times New Roman') 
...
%         Ax3=gca;
%         
% %         Legend_eachCinv2= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
%         Legend_eachCinv2= [(sample_info{x,5}),'-InAlAs',' (',sample_info{x,9},' ',num2str((sample_info{x,2})),' ',filename1(1:end-4),')']   
...
% %        xlim('auto')
%         set(Ax3,'XLim',[-1.5 7])
%         set(Ax3, 'XTick', [-1 0 1 2 3 4 5 6 7])
...
%          Legend_eachNV=[(sample_info{x,5}),'-InAlAs',' (',sample_info{x,9},' ',num2str((sample_info{x,2})),' ',filename1(1:end-4),')']   
...
% 
%          set(Ax4,'XLim',[-1.5 7])
%          set(Ax4, 'XTick', [-1 0 1 2 3 4 5 6 7])
...
%          Legend_eachNX=[(sample_info{x,5}),'-InAlAs',' (',sample_info{x,9},' ',num2str((sample_info{x,2})),' ',filename1(1:end-4),')']   
