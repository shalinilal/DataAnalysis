...
TitleStrCinv2=['1/C^{2}-V',' of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
TitleStrNV=['n-V',' of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
TitleStrNX=['n-x',' of ',sample_info{1,8},' diodes with p, pi, i doped InAlAs']   ;
% 
m=1;
...
%        set(Ax3, 'YTick', [10.^-2 10.^-1 10.^0 10.^1])
        set(Ax3,'XLim',[V_minlim V_maxlim])
...
%         Legend_eachCinv2= [(sample_info{x,5}),'-InAlAs (' ,num2str((sample_info{x,2})),' ', num2str(sample_info{x,6}),'-',filename1(1:end-4),')']
        ...
        PlotLinesCinv2(x)=plot(V,Cinv2,PlotLineSpec{x},'LineWidth',2);
         ...
         index_1V = find((V > 1)); % Voltage range over which doping is constant for gate-drain diodes only, will change for gate-source and source-drain
         index_2p5V = find((V > 2.5)); % Voltage range over which doping is constant for gate-drain diodes only, will change for gate-source and source-drain
...
%        set(Ax4,'YTick',[1e-6:100:100]) %mA
        set(Ax4, 'YTick', [10.^16 10.^17 10.^18 10.^19])
         set(Ax4,'XLim',[V_minlim V_maxlim])
