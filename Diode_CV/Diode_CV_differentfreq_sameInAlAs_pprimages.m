...
%         [C2,V2] = smoother(Cm,V,dCmin);
        ...
        N = (2 / (q*Es*Eo))./dCinv2;
        dN = interp1(N,linspace(1,length(N),100));
         
        X = 1e7*Es*Eo*Ap_Area./Cm; %nm
        dX = interp1(X,linspace(1,length(X),100));
...
%        set(Ax1, 'YTick', [10.^-6 10.^-5 10.^-4 10.^-3 10.^-2 10.^-1 10.^0 10.^1 10.^2])
        ...
        xlabel('V_{DG} (V)','FontSize',30,'FontName','Times New Roman') 
